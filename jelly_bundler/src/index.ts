...
const canvas = (document.getElementById('jelly-canvas') || document.querySelector('canvas')) as HTMLCanvasElement;
...
const camera = new CameraController(
  root,
  d.vec3f(0.02, 1.8, 1.4), // Moved closer from (0.024, 2.7, 1.9)
  d.vec3f(0, 0, -0.1), // Look slightly lower/closer
  d.vec3f(0, 1, 0),
  Math.PI / 4,
  width,
  height,
...
const renderBackground = (
  ...
  // Rail line alpha: only opaque where the rail is.
  const distToRail = rectangleCutoutDist(hitPosition.xz);
  const alpha = 1.0 - std.smoothstep(0.0, 0.005, distToRail);

  return d.vec4f(backgroundColor.xyz, alpha);
};
...
const raymarchFn = tgpu['~unstable'].fragmentFn({
  ...
  const rgb = std.tanh(color.xyz.mul(exposure)).mul(color.w);
...
const rayMarchPipeline = root['~unstable']
  ...
  .withFragment(raymarchFn, { format: presentationFormat })
  .createPipeline();
...
function render(timestamp: number) {
  frameCount++;
  // camera.jitter(); // Removed to fix vibration since we are not using TAA
